...
export const add = async function (user) {
  if (user instanceof User) await dbGate.post(mainURL, user);
};

export const Update = async function (user) {
  if (user instanceof User) await dbGate.update(mainURL, user.id, user);
};

export const Delete = async function (user) {
  if (user instanceof User) await dbGate.del(mainURL, user.id);
};
...
export const isLogged = async function () {
  ...
  if (id && (await getUserById(atob(id)))) {
    if (rememberMe) {
        return rememberMe == "true" ? 1 : 0;
    }
    return -1;
  }
};
